...
from core.imagelib import SegIEPolyType
from facelib import FaceType, LandmarksProcessor

is_windows = sys.platform[0:3] == 'win'
xseg_input_size = 256

def apply_directional_mask_modifiers(mask, top, bottom, left, right):
    """
    Applies directional expansion/erosion using cv2.dilate/erode with
    directional kernels and correctly specified anchor points.
    This is the definitive, non-shifting, non-inverted version.
    """
    # Make a copy to avoid modifying the original mask in place
    result_mask = mask.copy()

    # Vertical Processing
    if top > 0: # Expand Up
        kernel = np.ones( (top, 1), np.uint8)
        result_mask = cv2.dilate(result_mask, kernel, anchor=(0, 0) )
    elif top < 0: # Erode Top
        kernel = np.ones( (abs(top), 1), np.uint8)
        result_mask = cv2.erode(result_mask, kernel, anchor=(0, abs(top)-1) )

    if bottom > 0: # Expand Down
        kernel = np.ones( (bottom, 1), np.uint8)
        result_mask = cv2.dilate(result_mask, kernel, anchor=(0, abs(bottom)-1) )
    elif bottom < 0: # Erode Bottom
        kernel = np.ones( (abs(bottom), 1), np.uint8)
        result_mask = cv2.erode(result_mask, kernel, anchor=(0, 0) )

    # Horizontal Processing
    if left > 0: # Expand Left
        kernel = np.ones( (1, left), np.uint8)
        result_mask = cv2.dilate(result_mask, kernel, anchor=(0, 0) )
    elif left < 0: # Erode Left
        kernel = np.ones( (1, abs(left)), np.uint8)
        result_mask = cv2.erode(result_mask, kernel, anchor=(abs(left)-1, 0) )

    if right > 0: # Expand Right
        kernel = np.ones( (1, right), np.uint8)
        result_mask = cv2.dilate(result_mask, kernel, anchor=(abs(right)-1, 0) )
    elif right < 0: # Erode Right
        kernel = np.ones( (1, abs(right)), np.uint8)
        result_mask = cv2.erode(result_mask, kernel, anchor=(0, 0) )
...
def MergeMaskedFace (predictor_func, predictor_input_shape,
    ...
    # Generate a mask of pre-defined exclusion zones (e.g., beards, glasses)
    exclusion_mask = None
    if dfl_img is not None:
        seg_ie_polys = dfl_img.get_seg_ie_polys()
        if seg_ie_polys.has_polys():
            try:
                h, w = wrk_face_mask_a_0.shape[:2]
                temp_exclusion_mask_2d = np.zeros((h, w), dtype=np.float32)
                seg_ie_polys.overlay_mask(temp_exclusion_mask_2d, include_only_exclusion=True)

                if np.any(temp_exclusion_mask_2d > 0):
                    exclusion_mask = temp_exclusion_mask_2d[..., None]
            except TypeError:
                # This block will execute if the API call is wrong.
                # It prevents a crash but the feature will not work.
                # This indicates we need to inspect DFLIMG.py for the correct API.
                pass
    ...
    if 'raw' not in cfg.mode:
        ...
        # Combine old uniform modifier with new directional modifiers
        # Note: original erode_mask_modifier is inverted (positive erodes, negative dilates/expands)
        uniform_expand = -ero
        
        top    = cfg.expand_top_modifier    + uniform_expand
        bottom = cfg.expand_bottom_modifier + uniform_expand
        left   = cfg.expand_left_modifier   + uniform_expand
        right  = cfg.expand_right_modifier  + uniform_expand
        # Apply the combined directional modifiers
        wrk_face_mask_a_0 = apply_directional_mask_modifiers(wrk_face_mask_a_0, top, bottom, left, right)
    ...
    # Punch out the exclusion zones from the final processed mask
    if exclusion_mask is not None:
        wrk_face_mask_a_0 -= exclusion_mask
        wrk_face_mask_a_0 = np.clip(wrk_face_mask_a_0, 0, 1)
    ...
    if cfg.mode == 'original':
        ...
        if maxregion.size != 0:
            ...
            if min(lenx,leny) >= 4:
                ...
                # Prepare stats mask for color transfer
                stats_mask = None
                if cfg.color_transfer_mask_mode != 0:
                    if cfg.color_transfer_mask_mode == 1: # 'hull'
                        stats_mask = cv2.resize(dst_face_mask_a_0, (output_size, output_size), interpolation=cv2.INTER_LINEAR)
                        if stats_mask.ndim == 2:
                            stats_mask = stats_mask[...,None]
                    elif cfg.color_transfer_mask_mode == 2: # 'learned'
                        stats_mask = wrk_face_mask_area_a

                # Set source mask equal to target mask
                source_stats_mask = stats_mask

                if 'seamless' not in cfg.mode and cfg.color_transfer_mode != 0:
                    if cfg.color_transfer_mode == 1: #rct
                        prd_face_bgr = imagelib.reinhard_color_transfer (prd_face_bgr, dst_face_bgr, target_mask=stats_mask, source_mask=source_stats_mask)
